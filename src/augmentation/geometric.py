...
def apply_rotate(image: np.ndarray, angle: float = 25.0) -> np.ndarray:
    ...
    # Use borderMode to fill empty areas with white (255, 255, 255)
...
def apply_skew(
    image: np.ndarray, skx: float = 0.001, sky: float = 0.008) -> np.ndarray:
    """
    ...
    T_to_center = np.array([[1, 0, -cx], [0, 1, -cy], [0, 0, 1]], dtype=np.float32)
    P = np.array([[1, 0, 0], [0, 1, 0], [h20, h21, 1]], dtype=np.float32)
    T_back = np.array([[1, 0, cx], [0, 1, cy], [0, 0, 1]], dtype=np.float32)
    
    # The combined transformation matrix: translate to center, skew, then translate back.
...
def apply_shear(image: np.ndarray, kx: float, ky: float) -> np.ndarray:
    ...
    Positive `kx` shears rightward, negative leftward; `ky` shears downward/upward.
    """
...
def apply_radial_distortion(image: np.ndarray, k: float=0.3) -> np.ndarray:
    """
    Apply a random elastic distortion to the image.
    """
    h, w = image.shape[:2]
    cx, cy = w / 2.0, h / 2.0
